...
void ImagePickerAndroid::pickImage(QString sWord, QString sLang, QString sWord2, QString sLang2)
{
 ...
 // auto oRes = QtAndroidPrivate::requestPermissions(permissions);
...
*/
  ...
  auto permissionCallback = [&](const QtAndroid::PermissionResultMap & permissionResult)
  {
    ...
    {
      qDebug() << i.iter().key() << " Permission:" <<  (i.val() ==
  QtAndroid::PermissionResult::Denied ? "denied" : "granted"); if (i.val() ==
  QtAndroid::PermissionResult::Granted)
      {
        QAndroidJniObject ACTION_PICK =
  QAndroidJniObject::getStaticObjectField("android/content/Intent", "ACTION_PICK",
  "Ljava/lang/String;"); QAndroidJniObject EXTERNAL_CONTENT_URI =
  ...
  "(Ljava/lang/String;Landroid/net/Uri;)V", ACTION_PICK.object<jstring>(),
